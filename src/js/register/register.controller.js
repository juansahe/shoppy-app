					...
					if(validate($scope.user.email)){
						if(validate($scope.user.bornday)){
							if(validate($scope.user.password)){
								...
								$ionicLoading.show({ template: 'Debe ingresar una contraseña valida', noBackdrop: false, duration: 2000 });
							}
						}else{
							//si la fecha no se ingreso
							document.getElementById('bornday').style.border = "solid 1px red";
							$ionicLoading.show({ template: 'Debe ingresar una fecha de nacimiento', noBackdrop: false, duration: 2000 });
						}
					}else{
						//si el user name no existe
						document.getElementById('email').style.border = "solid 1px red";
						$ionicLoading.show({ template: 'Debe ingresar un correo electronico valido', noBackdrop: false, duration: 2000 });
					}
				}else{
					//si el correo no se ingreso
					document.getElementById('username').style.border = "solid 1px red";
					$ionicLoading.show({ template: 'Debe ingresar un nombre de usuario valido', noBackdrop: false, duration: 2000 });
				}
